...
function form_textarea($input_name, $label = '', $input_value = '', array $options = array()) {
    ...
    if ($options['type'] == "tinymce") {
        ...
        switch ($options['tinymce']) {
			...
			'table contextmenu directionality template paste textcolor emoticons responsivefilemanager youtube codesample ".($options['inline_editing'] ? " save " : "")."'
			],
			...
			toolbar3: 'hr pagebreak insertdatetime | link unlink anchor | image media | table charmap visualchars visualblocks emoticons | responsivefilemanager youtube',
