	...
	function render_forum_item($data, $i) {
		...
		switch ($data['forum_type']) {
			case '3':
				...
				break;
			default:
				echo "<div class='col-xs-12 col-sm-5' width=250>\n";
				
				echo "
				<a class='display-inline-block forum-link' href='".$data['forum_link']['link']."'>".$data['forum_link']['title']."</a>\n<span class='m-l-5'>".$data['forum_new_status']."</span><br/>";
				echo $data['forum_description'] ? "<div class='forum-description' style='line-height: 1;'><small>".$data['forum_description']."</small></div>\n" : '';
				echo ($data['forum_moderators'] ? "<span class='forum-moderators text-smaller'><strong>".$locale['forum_0007']."</strong>".$data['forum_moderators']."</span>\n" : "")."\n";
				if (isset($data['child'])) {
					echo "<div class='clearfix sub-forum'>\n";
				...
				echo "<div class='hidden-xs col-sm-3 col-md-3 p-l-0 p-r-0 text-center' width=350>\n";

			echo "<div class='display-inline-block forum-stats p-5 m-r-5 m-b-0'>\n";
			echo "<h4 class='text-bigger strong text-dark m-0'>".number_format($data['forum_threadcount'])."</h4>\n";
			echo "<span>".format_word($data['forum_threadcount'], $locale['fmt_thread'], 0)."</span>";
			echo "</div>\n";				
			echo "<div class='display-inline-block forum-stats p-5 m-r-5 m-b-0'>\n";
			echo "<h4 class='text-bigger strong text-dark m-0'>".number_format($data['forum_postcount'])."</h4>\n";
			echo "<span>".format_word($data['forum_postcount'], $locale['fmt_post'], 0)."</span>";
			echo "</div>\n";


				echo "</div><div class='forum-lastuser hidden-xs hidden-sm col-md-4' width=250>\n";
				
				if ($data['forum_lastpostid'] == 0) {
					echo $locale['forum_0005'];
				} else {
						
					echo "<span class='forum_profile_link' width=250>".$data['last_post']['profile_link']." ".$data['last_post']['time']."</span>\n";
					...
					echo str_replace(array("\r\n", "\r", "\n"), ' ', fusion_first_words(strip_tags($data['last_post']['message']), 3));
				}
				echo "</div>\n";
...
if (!function_exists('render_thread_item')) {
	function render_thread_item($data) {
		...
		echo "<div class='row m-0'>\n";
		echo "<div class='col-xs-12 col-sm-9 col-md-5 p-l-0'>\n";
		...
		echo "</div>\n"; // end grid
		echo "<div class='forum-lastuser hidden-xs hidden-sm col-md-4'>
			".$data['thread_last']['profile_link']." ".timer($data['thread_last']['time'])."<br/>
			".fusion_first_words(strip_tags($data['thread_last']['post_message']), 3)."
		</div>\n";
		echo "</div>\n";
		echo "</div>\n";
	}
