...
class Moderator {
    ...
    private function remove_thread() {
        ...
        if (self::verify_thread($this->thread_id) && self::verify_forum($this->forum_id)) {
            $res = dbquery("DELETE FROM ".DB_FORUM_POSTS." WHERE thread_id='".intval($this->thread_id)."'");
            if($res){
						$data['post_deleted'] = $res->rowCount();
						}
            $res2 = dbquery("DELETE FROM ".DB_FORUM_THREAD_NOTIFY." WHERE thread_id='".intval($this->thread_id)."'");
            if ($res2){
						$data['track_deleted'] = $res2->rowCount();
						}
