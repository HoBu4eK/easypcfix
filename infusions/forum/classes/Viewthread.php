		...
		if (!empty($thread_data)) {
			...
			if (iMOD) {
				...
				<div class='list-group-item'>\n
					...
					".form_button('move_posts', $locale['forum_0176'], $locale['forum_0176'], array('class' => 'btn-default btn-sm'))."
			...
			if ($this->getThreadPermission("can_reply") && \defender::safe()) {
				...
				if (!flood_control("post_datestamp", DB_FORUM_POSTS, "post_author='".$userdata['user_id']."'")) { // have notice
					$post_data = array(
						...
						'post_smileys' => isset($_POST['post_smileys']) ? 1 : 0,
	...
	private function get_thread_post() {
		...
		if ($this->thread_info['post_rows'] > 0) {
			...
			while ($pdata = dbarray($result)) {
				...
				// Format Post Message
				IF($pdata['post_smileys']==0){
					$post_message = nl2br(parsesmileys(parseubb($pdata['post_message'])));	
				}else{
					$post_message = nl2br(parseubb($pdata['post_message']));		
				}
