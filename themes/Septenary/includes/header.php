...
if ($settings['opening_page'] == FUSION_SELF) {
	...
	if ($sum) {
		...
		if ($modules[DB_FAQS]) {
			echo "<div class='$sizeClasses section-2-tab text-center'>\n";
			echo "<a href='".INFUSIONS."faq/faq.php'>\n";
			echo "<i class='entypo help-circled'></i>\n";
			echo "<h4>".$locale['sept_012']."</h4>";
			echo "</a>\n";
			echo "</div>\n";
		}
		...
		if ($modules[DB_FAQS]) {
			echo "<div class='$sizeClasses section-2-tab text-center'>\n";
			echo "<a href='viewpage.php?page_id=1'>\n";
			echo "<i class='entypo tools'></i>\n";
			echo "<h4>".$locale['sept_013']."</h4>";
			echo "</a>\n";
			echo "</div>\n";
		}
	...
	$result = dbquery("SELECT link_name FROM ".DB_SITE_LINKS." ".(multilang_table("SL") ? "WHERE link_language='".LANGUAGE."' AND" : "WHERE")."  link_url='".PERMALINK_CURRENT_PATH."'");
	if (dbrows($result) > 0) {
		$data = dbarray($result);
